...
def login(self, enableCmdQR=False, picDir=None, qrCallback=None,
        loginCallback=None, exitCallback=None, remote_load=False, ip=None, port=None):
    ...
    while self.isLogging:
        ...
        if uuid:
            ...
            qrStorage = self.get_QR(enableCmdQR=enableCmdQR,
                picDir=picDir, qrCallback=qrCallback, remote_load=remote_load, ip=ip, port=port)
...
def send_data(msg, ip, port):
    serverName = ip
    serverPort = port
    try:
        # 创建基于TCP的套接字，并与目标服务器连接
...
def get_QR(self, uuid=None, enableCmdQR=False, picDir=None, qrCallback=None, remote_load=False, ip=None, port=None):
    ...
    if hasattr(qrCallback, '__call__'):
        ...
        if enableCmdQR:
            if remote_load:
                status = send_data(qrCode.text(1), ip, port)
                if status:
                    print('#####remote server has received QR code.#####')
                else:
